        ...
        <c:forEach var="sectionEntry" items="${resume.sections}">
            ...
            <c:choose>
                <c:when test="${type=='OBJECTIVE'}">
                    <tr>
                        <td colspan="2">
                            <h3><%=((SimpleTextSection) section).getContent()%></h3>
                        </td>
                    </tr>
                </c:when>
                ...
                <c:when test="${type=='QUALIFICATION' || type=='ACHIEVEMENT'}">
                    <tr>
                        <td colspan="2">
                            <ul>
                                <c:forEach var="item" items="<%=((MarkedTextSection) section).getOrganizations()%>">
                                    <li>${item}</li>
                                </c:forEach>
                            </ul>
                        </td>
                ...
                <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
                    <c:forEach var="org" items="<%=((OrganizationSection) section).getOrganizations()%>">
                        ...
                        <c:forEach var="position" items="${org.positions}">
                            <jsp:useBean id="position" type="com.urise.webapp.model.Experience.Position"/>
                            <tr>
